...
import { parseEdgeTxLogs } from './parseEdgeTxLog'
...
describe('parseEdgeTxLogs', () => {
  afterEach(() => {
    vi.restoreAllMocks()
  })

  it('should parse valid CSV text correctly', async () => {
    const result = await parseEdgeTxLogs(validCsvText)

    expect(result).toBeDefined()
    expect(result.records.length).toBe(4)
    expect(result.startDate).toEqual(new Date('2000-01-01T00:09:16.080Z'))
    expect(result.endDate).toEqual(new Date('2000-01-01T00:09:17.440Z'))
